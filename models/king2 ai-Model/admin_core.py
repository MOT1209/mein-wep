...
class AdminCore:
    ...
    def _get_res_val(self, row, idx, key):
        """Helper to get value from a row regardless of whether it's a dict or tuple."""
    ...
    def get_live_chats(self, limit: int = 100, offset: int = 0, search: str = "") -> Dict:
        result = {"chats": [], "total": 0, "limit": limit, "offset": offset}
        if not self._conn:
            return result
        try:
            cur = self._cursor()
            if self.is_postgres:
                if search:
                    cur.execute(
                        "SELECT COUNT(*) as count FROM chat_history WHERE user_message ILIKE %s OR ai_response ILIKE %s",
                        (f"%{search}%", f"%{search}%")
                    )
                else:
                    cur.execute("SELECT COUNT(*) as count FROM chat_history")
                result["total"] = self._get_res_val(cur.fetchone(), 0, "count") or 0
                ...
                if search:
                    cur.execute(
                        "SELECT COUNT(*) as count FROM chat_history WHERE user_message LIKE ? OR ai_response LIKE ?",
                        (f"%{search}%", f"%{search}%")
                    )
                else:
                    cur.execute("SELECT COUNT(*) as count FROM chat_history")
            ...
            rows = cur.fetchall()
            print(f"[AdminCore] DB returned {len(rows)} rows for live chats. Total count: {result['total']}")
            
            result["chats"] = [
                {"id": self._get_res_val(r, 0, "id"), 
                 "username": self._get_res_val(r, 1, "username"), 
                 "user_message": self._get_res_val(r, 2, "user_message"),
                 "ai_response": self._get_res_val(r, 3, "ai_response"), 
                 "model": self._get_res_val(r, 4, "model"), 
                 "api_provider": self._get_res_val(r, 5, "api_provider"), 
                 "response_time": self._get_res_val(r, 6, "response_time"), 
                 "timestamp": self._get_res_val(r, 7, "timestamp")}
                for r in rows
            ]
            cur.close()
        except Exception as e:
            result["error"] = str(e)
            print(f"[AdminCore CRITICAL ERROR] get_live_chats error: {e}")
            try:
                self._conn.rollback()
            except:
                pass
    ...
    def get_live_stats(self) -> Dict:
        ...
        try:
            ...
            # Total Messages
            cur.execute("SELECT COUNT(*) as count FROM chat_history")
            stats["total_messages"] = self._get_res_val(cur.fetchone(), 0, "count") or 0

            # Total Users
            cur.execute("SELECT COUNT(DISTINCT username) as count FROM chat_history")
            stats["total_users"] = self._get_res_val(cur.fetchone(), 0, "count") or 0

            # Knowledge Base Size
            try:
                cur.execute("SELECT COUNT(*) as count FROM knowledge_base")
                stats["kb_size"] = self._get_res_val(cur.fetchone(), 0, "count") or 0
            except:
                self._conn.rollback()
                stats["kb_size"] = 0
            ...
            # Total API Calls
            try:
                cur.execute("SELECT SUM(request_count) as total FROM api_usage")
                stats["total_api_calls"] = self._get_res_val(cur.fetchone(), 0, "total") or 0
            except:
                self._conn.rollback()
            ...
            try:
                cur.execute("SELECT AVG(response_time) as avg_time FROM chat_history")
                avg_time = self._get_res_val(cur.fetchone(), 0, "avg_time")
                stats["avg_response_time"] = round(float(avg_time), 3) if avg_time else 0.0
            except:
                self._conn.rollback()
                stats["avg_response_time"] = 0.0

            stats["db_connected"] = True
            stats["db_type"] = "PostgreSQL" if self.is_postgres else "SQLite"

            cur.close()
            print(f"[AdminCore] get_live_stats returning successfully: {stats['total_messages']} msgs")
        except Exception as e:
            stats["db_error"] = str(e)
            print(f"[AdminCore CRITICAL ERROR] get_live_stats error: {e}")
            try:
                self._conn.rollback()
            except:
                pass
    ...
    # ─── Kaggle Status (pure SQL) ───────────────────────────────────────────

    def get_kaggle_status(self) -> Dict:
        """Reads Kaggle stats from site_stats table — no global variables."""
        status = {
        ...
        try:
            cur = self._cursor()
            if self.is_postgres:
                cur.execute("SELECT key_name, int_value FROM site_stats WHERE key_name IN ('algorithms_ingested','self_corrections','notebooks_synced','datasets_synced','training_phase')")
                for row in cur.fetchall():
                    status[row['key_name']] = row['int_value']
                cur.execute("""
                    ...
                    ORDER BY sync_date DESC LIMIT 20
                """)
                status["sync_logs"] = [
                    {
                        "id": r['id'],
                ...
                status["sync_logs"] = [
                    {
                        "id": r[0], "sync_date": str(r[1]), "topics_processed": r[2],
                        "algorithms_ingested": r[3], "datasets_synced": r[4],
                        "self_corrections": r[5], "status": r[6], "message": r[7],
            ...
            print(f"[AdminCore] get_kaggle_status error: {e}")
            try:
                self._conn.rollback()
            except:
                pass
